...
uint8_t HW_GetBatteryLevel( void ) 
{
  ...
  //measuredLevel = HW_AdcReadChannel( ADC_CHANNEL_VREFINT );
  measuredLevel = HW_AdcReadChannel( ADC_CHANNEL_0 );
  ...
  {
	  batteryLevelmV = (( (uint32_t) VDDA_VREFINT_CAL * measuredLevel )/ ADC_RESOLUTION );
	  //batteryLevelmV = 2 * batteryLevelmV;  // sending the battery level twice as received
  }
  /*else
  {
    batteryLevelmV= (( (uint32_t) VDDA_VREFINT_CAL * (*VREFINT_CAL ) )/ measuredLevel);
...
/*  if (batteryLevelmV > VDD_BAT)
  ...
  // To get value in V
  // example here 3.3v is 33
  return (uint8_t)(batteryLevelmV / 100);
}
